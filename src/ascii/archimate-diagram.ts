...
import { canvasToString, increaseSize, mkCanvas } from "./canvas.ts";
...
export function renderArchimateAscii(
	...
	const presentLayers: ArchiMateLayer[] = LAYER_ORDER.filter(
		(layer) =>
			diagram.layers.has(layer) && diagram.layers.get(layer)!.length > 0,
	);
...
function drawRelationship(
	...
	if (dy >= dx) {
		...
		if (label) {
			const midY = Math.floor((startY + endY) / 2);
			const labelX =
				(srcCX !== tgtCX ? Math.floor((srcCX + tgtCX) / 2) : srcCX) + 2;
			for (let i = 0; i < label.length; i++) {
				const lx = labelX + i;
				if (lx >= 0 && midY >= 0) {
		...
		if (goingRight) {
			for (let x = startX; x <= endX; x++) {
				if (x >= 0 && x < totalW && lineY < totalH)
					canvas[x]![lineY] = marker.lineH;
			}
		} else {
			for (let x = startX; x >= endX; x--) {
				if (x >= 0 && x < totalW && lineY < totalH)
		...
		if (srcCY !== tgtCY) {
			...
			for (let y = ly; y <= ry; y++) {
				if (midX >= 0 && midX < totalW && y < totalH)
					canvas[midX]![y] = marker.lineV;
			}
			// Horizontal from midX to target
			const targetY = tgtCY;
			if (goingRight) {
				for (let x = midX + 1; x <= endX; x++) {
					if (x >= 0 && x < totalW && targetY < totalH)
						canvas[x]![targetY] = marker.lineH;
				}
			} else {
				for (let x = midX - 1; x >= endX; x--) {
					if (x >= 0 && x < totalW && targetY < totalH)
