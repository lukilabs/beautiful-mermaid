...
export async function layoutArchiMateDiagram(
	...
	for (const layer of presentLayers) {
		...
		positionedLayers.push({
			...
			height:
				maxY - minY + ARCHIMATE.layerPadding * 2 + ARCHIMATE.layerLabelHeight,
		});
	}
	...
	if (positionedLayers.length > 0) {
		...
		const globalMaxRight = Math.max(
			...positionedLayers.map((l) => l.x + l.width),
		);
