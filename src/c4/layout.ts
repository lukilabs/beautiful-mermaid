...
export async function layoutC4Diagram(
	...
	const positionedElements: PositionedC4Element[] = diagram.elements.map(
		(element) => {
...
function calculateElementSize(element: C4Element): {
	width: number;
	height: number;
} {
	...
	const height = Math.max(
		C4.minBoxHeight,
		textLines * C4.lineHeight + C4.boxPadY * 2,
	);
