...
const REL_DIRECTIONS: ReadonlyMap<string, C4Relationship["direction"]> =
	new Map([
		["Rel_D", "D"],
		["Rel_U", "U"],
		["Rel_L", "L"],
...
export function parseC4(lines: string[]): C4Diagram {
	...
	const diagramType = (
		DIAGRAM_TYPES.has(firstLine) ? firstLine : "C4Context"
	) as C4DiagramType;
		...
		if (boundaryMatch) {
			...
			if (boundaryKind) {
				...
				if (boundaryStack.length > 0) {
					boundaryStack[boundaryStack.length - 1]!.childBoundaries.push(
						boundary,
					);
				} else {
